...
/**
 * Convert an image to what it will look like in a heatmap
 *
 * The image needs to be 25-50 pixels high.
 * To fit in the last 10 minutes graph, be about 120 pixels wide.
 * This uses the overall darkness of each pixel to choose a color from the heatmap.
 */
...
// Modifies the input pixels and returns them again. this is fine
export function convertPixelsToWhatItsGonnaLookLike({ pixels, histogramColors }: ConvertPixelsInput) {
...
export function approximateColorByNumberOfSpans(allPixels: Pixel[]): (d: Pixel) => CountOfSpans {
  return spaninate("decide how many pixels to send per color", (s) => {
    ...
    s.setAttribute("app.derivedDarknessDensityFunction", JSON.stringify(derivedDarknessToEventDensity));
...
function incrementEventDensity(distinctBluenesses: number[]) {
  otel.trace.getActiveSpan()?.setAttribute("app.bluenessCalculationStrategy", "increment");
...
function calculateDensitySmoothly(maxSpansAtOnePoint: number, distinctDarknesses: number[]): Record<number, number> {
  otel.trace.getActiveSpan()?.setAttribute("app.bluenessCalculationStrategy", "smooth");
  const maxBlueness = Math.max(...distinctDarknesses);
  const bluenessWidth = maxBlueness;
  const increaseInSpansPerBlueness = bluenessWidth === 0 ? 1 : maxSpansAtOnePoint / bluenessWidth;
  const bluenessFunction = (b: number) =>
    maxSpansAtOnePoint - Math.round((maxBlueness - b) * increaseInSpansPerBlueness);

  return Object.fromEntries(distinctDarknesses.map((b) => [b, bluenessFunction(b)]));
}
